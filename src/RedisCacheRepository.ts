...
export class RedisCacheRepository implements ICache<any> {
  ...
  constructor(redis: Redis, config: CacheConfig) {
    ...
    const userSegment = process.env.USERNAME ? process.env.USERNAME + keySeparator : '';
  ...
  private _getSetExpiration(options: CacheSetOptions = {}) {
    const ttl = typeof options?.ttlInSeconds === 'number' ? options.ttlInSeconds : this.ttlInSeconds ?? 0;
  ...
  public async get(key: string): Promise<any | null> {
    const data = await this.redis.get(this._getKey(key)).then((value: any) => this._decode(value));
    return data?.value || null;
  }

  public async getWithMeta(key: string): Promise<CacheRecord<any> | null> {
    return await this.redis.get(this._getKey(key)).then((value: any) => this._decode(value));
  }

  public async set(key: string, value: any, options?: CacheSetOptions | undefined): Promise<boolean> {
